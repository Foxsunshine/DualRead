...
>
> **修订（2026-04-25 / W5.5）**：原本"登录用户每次划词都上传到自家
> backend"的设计已改为 dual-channel —— 划词永远本地 Google MT，AI 精翻
> 才走 backend 且需用户**显式**触发。详见 `docs/v3-product-design.md`。
> 这个修订对 CWS 审查是**净利好**：
> - 划词文本上传到自家 backend 的频次大幅下降（仅 AI 精翻才上传）
> - 用户可控（Settings 全局开关 + 气泡按钮，未授权前不上传）
> - 隐私政策措辞更易写："仅在你点击 ✨ AI 精翻时上传"
>
> 下文风险评估按 v3.1 原版（每次都上传）保守评估，实际 W5.5+ 风险更低。
