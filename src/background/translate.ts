...
// error-normalised. Both the side panel (via `useSelection`) and the
// bubble (via content/clickTranslate) route through here. Duplicate
// concurrent requests for the same (text, target) pair both hit the
// network once because the second request is sent *after* the first
// response has written to the cache — but if both fire within the
// first network round-trip, both will call `translateWithGoogle`.
// Accepted: Google's endpoint is cheap and the race window is small
// (~200 ms); deduplicating in-flight promises would require keeping
// promise state across service-worker evictions, which is not worth
// the complexity at v1.1's scale.
//
// W5.5 (post-mortem): an earlier commit (24a25f0) routed signed-in
// users through the Railway backend's /translate. Real-world latency
// (~1-3s warm, 10s+ cold-start) was unacceptable for the high-frequency
// click-translate path. Per docs/v3-product-design.md, click-translate
// is the "fast" channel that always stays local; the backend's value
// (RAG + style polish) is reserved for the explicit "AI 精翻" surface
// landing in Phase 2 W6+. So this function is local-only again.
// `requestTranslate()` in shared/api.ts stays as a dormant helper for
// the agent path's internal baseline-translate node.
